...
export async function runAuthCommand(args: string[]): Promise<number> {
  ...
  const timeoutMs = Number(process.env.POLAR_AUTH_TIMEOUT_MS ?? 300_000);

  const result = await waitForOAuthCode(redirect, state, timeoutMs, async (url) => {
    if (!json) {
      console.log("Polar MCP - Authorization");
      console.log("");
      if (noOpen) {
        console.log("Open this URL manually:");
        console.log(`  ${url}`);
      } else {
        console.log("Opening Polar authorization in your browser...");
      }
      console.log("");
      console.log("Steps");
      console.log("  1. Approve access in the browser tab that opens.");
      console.log("  2. Polar will redirect to the local callback.");
  ...
  const output = {
    ...
    next_step: "Run `polar-mcp-server doctor`, then add the MCP server to your agent."
  };
...
function waitForOAuthCode(
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      server.close();
      reject(new Error("Timed out waiting for POLAR OAuth callback."));
    }, timeoutMs);
    ...
    const server = createServer((req, res) => {
      try {
        ...
        if (error) throw new Error(`Polar authorization failed: ${error}`);
        if (!code) throw new Error("POLAR callback did not include a code.");
        if (state !== expectedState) throw new Error("POLAR callback state mismatch.");
...
<head>
  ...
  <title>Polar connected - Delx Wellness</title>
  <style>
...
<body>
  ...
  <ol>
    ...
    <li>Run <code>polar-mcp-server doctor</code> to verify the setup.</li>
