...
import { writeLocalConfig, type LocalPolarConfig } from "../services/local-config.js";
...
export async function runSetupCommand(args: string[]): Promise<number> {
  ...
  const config: LocalPolarConfig = {
    POLAR_CLIENT_ID: options.clientId,
    POLAR_CLIENT_SECRET: options.clientSecret,
    POLAR_REDIRECT_URI: options.redirectUri,
    POLAR_SCOPES: DEFAULT_SCOPES.join(" "),
    POLAR_PRIVACY_MODE: options.privacyMode
  };
  if (options.cache) config.POLAR_CACHE = options.cache;
  if (options.tokenPath) config.POLAR_TOKEN_PATH = options.tokenPath;
  if (options.cachePath) config.POLAR_CACHE_PATH = options.cachePath;
  ...
  if (options.json) console.log(JSON.stringify(setupOutput, null, 2));
    ...
    console.log("Polar MCP setup saved.");
    ...
    console.log("Secrets were saved only in the local Polar MCP config file.");
  }
...
async function parseSetupOptions(args: string[]): Promise<SetupOptions> {
  ...
  const clientId = required(answers, "client-id", "Polar Client ID");
  const clientSecret = required(answers, "client-secret", "Polar Client Secret");
...
async function promptForMissing(flags: Map<string, string>): Promise<Map<string, string>> {
  ...
  try {
    ...
    if (!merged.has("client-id")) merged.set("client-id", (await firstPrompt.question("Polar Client ID: ")).trim());
  } finally {
    firstPrompt.close();
  }
  if (!merged.has("client-secret")) merged.set("client-secret", await promptHidden("Polar Client Secret: "));

  const secondPrompt = createPromptInterface({ input, output });
  try {
    if (!merged.has("redirect-uri")) merged.set("redirect-uri", (await secondPrompt.question("Polar Redirect URI [http://127.0.0.1:3000/callback]: ")).trim() || "http://127.0.0.1:3000/callback");
...
function writeClientConfig(client: AgentClientName, homeDir: string): ClientConfigResult {
  ...
  const path = join(homeDir, ".polar-mcp", "mcp-configs", `${client}.json`);
...
function mergeClaudeConfig(homeDir: string): string {
  const path = process.platform === "darwin"
    ...
    : join(homeDir, ".polar-mcp", "mcp-configs", "claude-desktop.json");
  mkdirSync(dirname(path), { recursive: true, mode: 0o700 });
  ...
  const next = {
    ...
    mcpServers: {
      ...mcpServers,
      polar: mcpConfigSnippet().mcpServers.polar
    }
  };
...
function writeHermesClientConfig(homeDir: string): ClientConfigResult {
  ...
  return {
    ...
    warnings: [
      "After editing Hermes MCP config, use `/reload-mcp` or `hermes mcp test polar`; do not restart the Hermes gateway for normal Polar data access.",
...
function mergeHermesConfig(configPath: string): string | undefined {
  ...
  if (/polar-mcp-unofficial|polar-mcp-server|polar-mcp/.test(existing) && /^\s*polar\s*:/m.test(existing)) {
    if (existing.includes(PINNED_NPM_PACKAGE)) return undefined;
    const backupPath = backupConfig(configPath);
    const updated = existing.replace(/polar-mcp-unofficial(?:@\d+\.\d+\.\d+)?/g, PINNED_NPM_PACKAGE);
  ...
  const backupPath = backupConfig(configPath);
  const next = existing.trimEnd().length ? addHermesPolarBlock(existing) : snippet;
  writeFileSync(configPath, ensureReloadHint(next), { mode: 0o600 });
  chmodSync(configPath, 0o600);
  return backupPath;
}

function addHermesPolarBlock(existing: string): string {
  const serverBlock = [
    "  polar:",
    "    command: npx",
    "    args:",
...
function backupConfig(path: string): string {
  const backupPath = `${path}.bak-polar-mcp-${new Date().toISOString().replace(/[-:]/g, "").replace(/\..+/, "Z")}`;
...
function setupNextStep(client: AgentClientName, noAuth: boolean): string {
  const auth = noAuth ? "Run `polar-mcp-server auth`, then " : "";
  if (client === "hermes") {
    return `${auth}run \`polar-mcp-server doctor --client hermes\`, then use \`/reload-mcp\` or \`hermes mcp test polar\`.`;
  }
  return `${auth}run \`polar-mcp-server doctor\`.`;
}
