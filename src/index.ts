...
async function runHttp(): Promise<void> {
  const app = express();
  const host = process.env.POLAR_MCP_HOST ?? "127.0.0.1";
  const port = Number(process.env.POLAR_MCP_PORT ?? 3000);
  const allowedOrigin = process.env.POLAR_MCP_ALLOWED_ORIGIN ?? `http://${host}:${port}`;
...
if (cliResult !== undefined) {
  ...
  const transport = process.env.POLAR_MCP_TRANSPORT ?? (args.has("--http") ? "http" : "stdio");
