...
export const CollectionInputSchema = z.object({
  ...
  before: DateTimeSchema.describe("Only return Polar records before this time. Converted to Polar's exclusive to query parameter."),
...
export const IdInputSchema = z.object({
  id: z.union([z.string().min(1), z.number().int().positive()]).describe("Polar resource id."),
  privacy_mode: PrivacyModeSchema,
  response_format: ResponseFormatSchema
}).strict();

export const RouteInputSchema = z.object({
  route_id: z.string().min(1).describe("Polar route id returned by a calendar or training-session record."),
  privacy_mode: PrivacyModeSchema,
  response_format: ResponseFormatSchema
}).strict();
...
export const AuthUrlInputSchema = z.object({
  ...
  scopes: z.array(z.string()).optional().describe("Optional scope override. Defaults to read-only Polar scopes used by this server."),
  response_format: ResponseFormatSchema
}).strict();
...
export const WellnessContextInputSchema = z.object({
  days: z.number().int().min(1).max(30).default(7).describe("Lookback window for normalized Polar wellness context."),
  timezone: z.string().min(1).max(80).default("UTC").describe("IANA timezone used only for display, e.g. America/New_York."),
...
export const ConnectionStatusOutputSchema = z.object({
  ...
  client_checks: z.object({
    hermes: z.object({
      ...
      polar_server_configured: z.boolean(),
