...
import { DEFAULT_SCOPES, POLAR_DEVELOPER_PORTAL_URL, PINNED_NPM_PACKAGE } from "../constants.js";
import type { PrivacyMode, PolarTokenSet } from "../types.js";
...
export interface HermesClientCheck {
  ...
  polar_server_configured: boolean;
...
const REQUIRED_ENV = ["POLAR_CLIENT_ID", "POLAR_CLIENT_SECRET", "POLAR_REDIRECT_URI"];
...
export async function buildConnectionStatus(options: ConnectionStatusOptions = {}): Promise<ConnectionStatus> {
  ...
  return {
    ok,
    ready_for_polar_api: ready,
    client: options.client,
    node: {
      version: process.versions.node,
      supported: nodeSupported
    },
    privacy_mode: parsePrivacyMode(value("POLAR_PRIVACY_MODE")),
    ...
    oauth,
    cache: {
      enabled: parseBool(value("POLAR_CACHE")),
      path: cachePath
    },
...
async function inspectToken(path: string, nowSeconds: number): Promise<ConnectionStatus["token"]> {
  try {
    ...
    const token = JSON.parse(text) as Partial<PolarTokenSet>;
...
async function inspectHermesClient(homeDir: string): Promise<HermesClientCheck> {
  ...
  const skillPath = join(homeDir, ".hermes", "skills", "polar-mcp", "SKILL.md");
  const base: Omit<HermesClientCheck, "recommendations"> = {
    config_path: configPath,
    config_exists: false,
    polar_server_configured: false,
    package_pinned: false,
    skill_path: skillPath,
    skill_installed: false,
    direct_tool_prefix: "mcp_polar_",
  ...
  try {
    ...
    const check = {
      ...
      polar_server_configured: /polar-mcp-unofficial|polar-mcp-server|polar-mcp/.test(configText) && /^\s*polar\s*:/m.test(configText),
      package_pinned: /polar-mcp-unofficial@\d+\.\d+\.\d+/.test(configText),
...
function buildHermesRecommendations(check: Omit<HermesClientCheck, "recommendations">): string[] {
  ...
  if (!check.config_exists) {
    recommendations.push("Run `polar-mcp-server setup --client hermes --no-auth` to create a Hermes MCP config and local Hermes skill.");
  } else if (!check.polar_server_configured) {
    recommendations.push("Add a `polar` MCP server block to `~/.hermes/config.yaml`.");
  }
  if (check.config_exists && check.polar_server_configured && !check.package_pinned) {
  ...
  if (!check.skill_installed) {
    recommendations.push("Install the Hermes skill at `~/.hermes/skills/polar-mcp/SKILL.md` so agents prefer direct MCP tools over terminal workarounds.");
  }
  ...
  recommendations.push("After Hermes config changes, use `/reload-mcp` or `hermes mcp test polar`; do not run `hermes gateway restart` for normal Polar data access.");
...
function buildOAuthScopeStatus(token: ConnectionStatus["token"]): ConnectionStatus["oauth"] {
  ...
  return {
    ...
    scope_status: missingRecommendedScopes.length === 0 ? "ok" : "missing_recommended",
      activity_tools_ready: granted.has("activity:read"),
      profile_tools_ready: granted.has("profile:read")
  };
}
...
function buildNextSteps(input: {
  ...
  for (const name of input.missingEnv) {
    steps.push(`Set ${name}. Create a Polar AccessLink client at ${POLAR_DEVELOPER_PORTAL_URL} if needed.`);
  }
  if (input.redirectUri && !input.automaticAuthSupported) {
    steps.push("For one-command auth, set POLAR_REDIRECT_URI to a local callback such as http://127.0.0.1:3000/callback.");
  }
  if (!input.token.exists) {
    steps.push("Run `polar-mcp-server auth` to authorize Polar and save local tokens.");
    ...
    steps.push("Re-authorize with `polar-mcp-server auth`; the current token is expired and has no refresh token.");
  }
  if (input.oauth.scope_status === "missing_recommended") {
    steps.push(`Re-authorize with \`polar-mcp-server auth\` to grant recommended scopes: ${input.oauth.missing_recommended_scopes.join(" ")}.`);
  } else if (input.oauth.scope_status === "unknown" && input.token.exists && input.token.readable) {
    steps.push("Token scope is not recorded locally. If Polar data tools fail with permission errors, re-authorize with `polar-mcp-server auth`.");
  }
  if (steps.length === 0) steps.push("Ready. Add this MCP server to your agent and start with polar_daily_summary.");
  return steps;
}
