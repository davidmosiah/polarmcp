...
export type LocalPolarConfig = Partial<Record<LocalConfigKey, string>>;

export interface LocalConfigReadResult {
  path: string;
  exists: boolean;
  values: LocalPolarConfig;
  permissions?: string;
...
export function getLocalConfigPath(homeDir = homedir()): string {
  return join(homeDir, ".polar-mcp", "config.json");
}
...
export function writeLocalConfig(values: LocalPolarConfig, homeDir = homedir()): string {
  const path = getLocalConfigPath(homeDir);
