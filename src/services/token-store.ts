...
import type { PolarTokenSet } from "../types.js";
...
export class TokenStore {
  ...
  async read(): Promise<PolarTokenSet | null> {
    try {
      const text = await fs.readFile(this.tokenPath, "utf8");
      return JSON.parse(text) as PolarTokenSet;
    } catch (error) {
  ...
  async write(tokens: PolarTokenSet): Promise<void> {
