...
const pinnedPackage = `polar-mcp-unofficial@${packageJson.version}`;
const dir = mkdtempSync(join(tmpdir(), 'polar-mcp-hermes-agent-'));

const client = new Client({ name: 'polar-mcp-hermes-agent-test', version: '0.0.0' });
...
try {
  ...
  assert.ok(toolNames.includes('polar_agent_manifest'), 'Hermes-ready agent manifest tool should be registered.');

  const resources = await client.listResources();
  const resourceUris = resources.resources.map((resource) => resource.uri);
  assert.ok(resourceUris.includes('polar://agent-manifest'), 'Agent manifest resource should be registered.');

  const manifestResult = await client.callTool({
    name: 'polar_agent_manifest',
  ...
  assert.equal(manifest.hermes.tool_name_prefix, 'mcp_polar_');
  ...
  assert.ok(manifest.hermes.common_tool_names.includes('mcp_polar_polar_connection_status'));
  ...
  const manifestResource = await client.readResource({ uri: 'polar://agent-manifest' });
  const text = manifestResource.contents[0]?.text ?? '';
  assert.match(text, /mcp_polar_polar_connection_status/);
  ...
  assert.ok(setupPayload.hermes_skill_path.endsWith('.hermes/skills/polar-mcp/SKILL.md'));
  ...
  assert.match(readFileSync(setupPayload.hermes_skill_path, 'utf8'), /mcp_polar_polar_connection_status/);
  ...
  assert.equal(doctorPayload.client_checks.hermes.polar_server_configured, true);
  ...
  const mergeDir = mkdtempSync(join(tmpdir(), 'polar-mcp-hermes-merge-'));
  ...
  assert.match(mergedConfig, /polar:/);
